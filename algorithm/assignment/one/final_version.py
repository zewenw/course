def convertRectangleToEnvelope(rect):
    """
    change the representation of rectangle coordinates, for example,
    the new rectangel is (x1, x2, h), after transformation,
    it becomes [(x1, h), (x2, 0)], that means it becomes an envelope
...
def mergeRectToEnvelope(envelope, rect):
    ...
    :variable rect_envelope: the new rectangle need needs
     to be converted to envelope
...
def mergeTwoEnvelopes(first_env, second_env):
    ...
    while i < len_1 and j < len_2:
        ...
        if min_x == first_env[i][0] and min_x != second_env[j][0]:
            ...
            i += 1
            # if the height is different from previous height, then append this coordinate pair to the result
